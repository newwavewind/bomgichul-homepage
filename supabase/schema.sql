...
declare
  memo_author_id uuid;
begin
  select user_id into memo_author_id
  ...
  if memo_author_id is not null and memo_author_id <> new.user_id then
    insert into public.notifications (
      ...
      memo_id, memo_comment_id, type
    ) values (
      memo_author_id, new.user_id, null, null,
      new.memo_id, new.id, 'memo_comment'
    )
...
create policy "본인 로그인 기록만 조회"
  on public.user_daily_logins for select using (auth.uid() = user_id);
...
create policy "게시글 좋아요 누구나 조회" on public.post_likes for select using (true);
create policy "로그인 사용자 게시글 좋아요" on public.post_likes for insert
  with check (
    auth.uid() = user_id
    and exists (select 1 from public.posts p where p.id = post_id and p.author_id <> auth.uid())
  );
create policy "본인 게시글 좋아요만 취소" on public.post_likes for delete using (auth.uid() = user_id);
...
create policy "댓글 좋아요 누구나 조회" on public.comment_likes for select using (true);
create policy "로그인 사용자 댓글 좋아요" on public.comment_likes for insert
  with check (
    auth.uid() = user_id
    and exists (select 1 from public.comments c where c.id = comment_id and c.author_id <> auth.uid())
  );
create policy "본인 댓글 좋아요만 취소" on public.comment_likes for delete using (auth.uid() = user_id);
...
returns table (
  user_id uuid, login_days bigint, post_count bigint,
  comment_count bigint, likes_received bigint, score bigint
)
language sql stable security definer set search_path = public as $$
  with requested as (select distinct unnest(target_user_ids[1:100]) as user_id),
  login_totals as (
    select l.user_id, count(*)::bigint as count from public.user_daily_logins l
    join requested r on r.user_id = l.user_id group by l.user_id
  ),
  all_posts as (
    select p.author_id as user_id, 2::bigint as points from public.posts p join requested r on r.user_id = p.author_id
    union all select p.user_id, 3::bigint from public.concept_community_posts p join requested r on r.user_id = p.user_id
    union all select m.user_id, 2::bigint from public.question_public_memos m join requested r on r.user_id = m.user_id
  ),
  post_totals as (select user_id, count(*)::bigint as count, sum(points)::bigint as score from all_posts group by user_id),
  all_comments as (
    select c.author_id as user_id from public.comments c join requested r on r.user_id = c.author_id
    union all select c.user_id from public.concept_community_post_comments c join requested r on r.user_id = c.user_id
    union all select c.user_id from public.question_public_memo_comments c join requested r on r.user_id = c.user_id
  ),
  comment_totals as (select user_id, count(*)::bigint as count from all_comments group by user_id),
  all_received_likes as (
    select p.author_id as user_id from public.post_likes l join public.posts p on p.id = l.post_id join requested r on r.user_id = p.author_id
    union all select c.author_id from public.comment_likes l join public.comments c on c.id = l.comment_id join requested r on r.user_id = c.author_id
    union all select p.user_id from public.concept_community_post_likes l join public.concept_community_posts p on p.id = l.post_id join requested r on r.user_id = p.user_id where l.user_id <> p.user_id
    union all select m.user_id from public.question_public_memo_likes l join public.question_public_memos m on m.id = l.memo_id join requested r on r.user_id = m.user_id where l.user_id <> m.user_id
  ),
  like_totals as (select user_id, count(*)::bigint as count from all_received_likes group by user_id)
  select r.user_id,
    coalesce(ld.count, 0)::bigint, coalesce(pt.count, 0)::bigint,
    coalesce(ct.count, 0)::bigint, coalesce(lt.count, 0)::bigint,
    (coalesce(ld.count, 0) + coalesce(pt.score, 0) + coalesce(ct.count, 0) + coalesce(lt.count, 0) * 2)::bigint
  from requested r
...
create or replace function public.get_community_like_state(target_post_id uuid, target_comment_ids uuid[])
returns table (target_type text, target_id uuid, like_count bigint, liked_by_viewer boolean)
language sql stable security definer set search_path = public as $$
  select 'post'::text, target_post_id, count(l.user_id)::bigint,
    coalesce(bool_or(l.user_id = auth.uid()), false)
  from (select 1) seed left join public.post_likes l on l.post_id = target_post_id
  union all
  select 'comment'::text, ids.id, count(l.user_id)::bigint,
    coalesce(bool_or(l.user_id = auth.uid()), false)
  from (select distinct unnest(target_comment_ids[1:100]) as id) ids
