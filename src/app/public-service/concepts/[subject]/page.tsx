...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!data) return {};
  return buildPageMetadata({ title: `${data.subject.label} 기출 all-in-one`, description: `9급 공무원 ${data.subject.label} 핵심 개념 ${data.concepts.length}개를 기출 해설 중심으로 정리했습니다. 단원별 개념과 관련 기출문제를 무료로 학습하세요.`, path: `/public-service/concepts/${subjectId}` });
}
