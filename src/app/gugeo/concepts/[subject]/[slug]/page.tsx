...
export function generateStaticParams() {
  return trackConceptStaticParams(api, GUGEO_SUBJECT_IDS);
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return trackConceptDetailMetadata(GUGEO_TRACK, api, subject, slug);
}

export default async function Page({ params }: Props) {
  const { subject, slug } = await params;
  return <TrackConceptDetailPage track={GUGEO_TRACK} api={api} subjectId={subject} slug={slug} />;
}
