import { getExamCountdown, getDaysUntilExam, formatKoreanDate } from "@/lib/exam";
import { TintedAccentCard } from "@/components/ui/Card";
import { Tag } from "@/components/ui/Tag";

export function DDayPanel() {
  const { exam, days, label, formattedDate } = getExamCountdown();
  const regDays = getDaysUntilExam(exam.registrationStart);
    ...
    <TintedAccentCard className="relative overflow-hidden">
      <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="font-display text-body-sm text-smoke">공인중개사 자격시험</p>
      ...
      <div className="mt-6 flex flex-wrap gap-3 border-t border-mist/40 pt-5">
        <div className="rounded-[var(--radius-tags)] bg-paper/80 px-3 py-1.5">
          ...
          <span className="font-display text-body-sm font-medium text-ink">
            {formatKoreanDate(exam.registrationStart).replace(/^\d+년 /, "")} ~{" "}
            {exam.registrationEnd.slice(5).replace("-", ".")}
          </span>
          {regDays > 0 && (
        ...
        <div className="rounded-[var(--radius-tags)] bg-paper/80 px-3 py-1.5">
          ...
          <span className="font-display text-body-sm font-medium text-ink">
            {formatKoreanDate(exam.resultDate).replace(/^\d+년 /, "")}
          </span>
        </div>
      </div>
    </TintedAccentCard>
