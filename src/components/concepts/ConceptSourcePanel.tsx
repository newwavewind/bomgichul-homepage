export type ConceptSource = { title: string; locator?: string; status?: string; type?: string };
...
export function ConceptSourcePanel({ sources = [], examLabels = [], amendmentNotice }: { sources?: ConceptSource[]; examLabels?: string[]; amendmentNotice?: string }) {
  const verifiedDates = [...new Set(sources.map((source) => dateIn(source.title)).filter(Boolean))];
  return <aside className="mt-6 rounded-2xl border border-mist bg-surface p-5" aria-label="콘텐츠 출처와 검수 정보">
    ...
    {sources.length ? <ul className="mt-3 space-y-2">{sources.map((source, index) => <li key={`${source.title}-${index}`} className="font-display text-body-sm text-smoke"><strong className="text-ink">{source.type === "law" ? "법령" : source.type === "case" ? "판례" : "집필·검수"}</strong> · {source.title}{source.locator ? <span className="text-fog"> · {source.locator}</span> : null}{source.status === "verified" ? <span className="ml-2 text-[#087f6d]">검수 완료</span> : null}</li>)}</ul> : <p className="mt-3 font-display text-body-sm text-smoke">연결된 공식 기출과 해설을 기준으로 구성했으며, 개별 법령·판례 출처는 확인되는 항목부터 공개합니다.</p>}
