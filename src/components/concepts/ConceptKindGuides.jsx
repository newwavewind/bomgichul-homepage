  ...
  return (
    <section className="cx-section">
      <h3 className="cx-section__label">
        ...
        {badge ? (
          <span className="cx-correct-pack-badge" title="O 선지와, 틀린 선지를 해설로 고친 옳은 문장만 모았습니다">
            {badge}
          </span>
        ) : null}
...
export function ConceptVisualGuide({ guide }) {
...
function TwentySecondSixGuide({ guide }) {
  ...
  if (guide.kind === 'mortgage-tether-map') {
    ...
    label = '부기등기 순위 스택'
    body = <><div className="cx-registration-stack">{guide.stack.map(x=><section className={x.number.includes('-')?'is-sub':''} key={x.number}><b>{x.number}</b><strong>{x.label}</strong><span>{x.priority}</span></section>)}</div><div className="cx-subreg-types">{guide.types.map(x=><section key={x.event}><strong>{x.event}</strong><b>{x.form}</b></section>)}</div><ConceptStudySheet guide={guide} showIntro={false} /></>
...
function TwentySixthSixGuide({ guide }) {
  ...
  if (guide.kind === 'attachment-independence-lab') {
    ...
    label = '지상경계점 현장 필드북'
    body = <><div className="cx-boundary-sheet">{guide.sheet.map(x=><section key={x.field}><strong>{x.field}</strong><span>{x.value}</span></section>)}</div><ConceptStudySheet guide={guide} showIntro={false} /></>
...
function TwentySeventhSixGuide({ guide }) {
  ...
  if (guide.kind === 'mortgage-value-transformer') {
    ...
    body = <><div className="cx-title-actors">{guide.actors.map(x=><section key={x.actor}><strong>{x.actor}</strong><span>{x.role}</span></section>)}</div><div className="cx-title-lines">{guide.lines.map(x=><section key={`${x.from}-${x.to}`}><b>{x.from}</b><i>→</i><b>{x.to}</b><strong>{x.line}</strong></section>)}</div><ConceptStudySheet guide={guide} showIntro={false} /></>
...
function FiftyEighthSixGuide({ guide }) {
  ...
  if (guide.kind === 'joint-committee-scope-board') {
    ...
    label = '개발행위허가 기준 필터'
    body = <><ConceptStudySheet guide={guide} showIntro={false} /><aside className="cx-b58-note is-alert">{guide.caution}</aside></>
...
function SixtyThirdSixGuide({ guide }) {
  ...
  if (guide.kind === 'bond-redemption-board') {
    ...
    label = '재건축 안전진단판'
    body = <><div className="cx-anchor-date">{guide.timing}</div><div className="cx-easement-extra">{guide.exempt}</div><ConceptStudySheet guide={guide} showIntro={false} /><div className="cx-easement-extra">{guide.reportRule}</div><aside className="cx-b63-note is-alert">{guide.caution}</aside></>
...
function EighteenthSixGuide({ guide }) {
  ...
  if (guide.kind === 'exchange-contract-scales') {
    ...
    body = <><ConceptStudySheet guide={guide} showIntro={false} /><div className="cx-plan-line">{guide.plan.map((x,i)=><span key={x}><b>{i+1}</b>{x}</span>)}</div></>
...
function FifteenthSixGuide({ guide }) {
  ...
  if (guide.kind === 'earnest-money-exit-gate') {
    ...
    body = <><div className="cx-reward-funnel">{guide.funnel.map((x,i)=><section key={x} style={{width:`${100-i*10}%`}}><b>{i+1}</b>{x}</section>)}</div><ConceptStudySheet guide={guide} showIntro={false} /><div className="cx-target-tags">{guide.targets.map(x=><span key={x}>{x}</span>)}</div></>
...
function FourteenthSixGuide({ guide }) {
  ...
  if (guide.kind === 'rescission-signal-console') {
    ...
    label = '지상경계 결정기준'
    body = <ConceptStudySheet guide={guide} />
...
function TwelfthSixGuide({ guide }) {
  ...
  if (guide.kind === 'passage-route-map') {
    label = '주위토지통행권 경로도'
    body = <><div className="cx-passage-route">{guide.route.map((x,i)=><span key={x}><b>{i+1}</b>{x}</span>)}</div><ConceptStudySheet guide={guide} showIntro={false} /></>
    ...
    label = '개설등록 결격기간 자물쇠'
    body = <><div className="cx-disqualification-locks">{guide.locks.map(x=><section key={x.event}><strong>{x.event}</strong><small>{x.start}부터</small><b>{x.wait}</b></section>)}</div><div className="cx-no-lock">{guide.noLock.map(x=><span key={x}>{x}</span>)}</div></>
...
function EighthSixGuide({ guide }) {
  ...
  if (guide.kind === 'sale-fruits-timeline') {
    ...
    body = <><div className="cx-sale-timeline">{guide.moments.map(x=><section key={x.at}><b>{x.at}</b><strong>{x.title}</strong><span>{x.detail}</span><small>{x.article}</small></section>)}</div><aside className="cx-special-caution"><b>지급기 예외</b><span>{guide.exception}</span></aside></>
...
function SeventhSixGuide({ guide }) {
  ...
  if (guide.kind === 'sub-agency') {
    ...
    label = '원리금 상환 장부'
    body = <><div className="cx-amort-formulas">{guide.formulas.map(x=><section key={x.name}><strong>{x.name}</strong><span>{x.formula}</span></section>)}</div><div className="cx-amort-table"><div className="cx-amort-table__head"><b>회차</b><b>기초잔액</b><b>이자</b><b>원금</b><b>기말잔액</b></div>{guide.rows.map(x=><div className="cx-amort-table__row" key={x.round}><strong data-col="회차">{x.round}</strong><span data-col="기초잔액">{x.open}</span><span data-col="이자">{x.interest}</span><span data-col="원금">{x.principal}</span><span data-col="기말잔액">{x.close}</span></div>)}</div><aside className="cx-special-caution"><b>계산 기준</b><span>{guide.caution}</span></aside></>
...
function SixthSixGuide({ guide }) {
  ...
  if (guide.kind === 'risk-allocation') {
    ...
    label = '일반·전속중개계약 비교판'
    body = <><div className="cx-contract-table"><div className="cx-contract-table__head"><b>구분</b><b>일반</b><b>전속</b></div>{guide.rows.map(x=><div className="cx-contract-table__row" key={x.axis}><strong>{x.axis}</strong><span data-col="일반">{x.general}</span><span data-col="전속">{x.exclusive}</span></div>)}</div><div className="cx-period-rule"><span>기간 약정 없음</span><strong>3개월</strong><i>／</i><span>별도 약정 있음</span><strong>약정기간</strong></div><aside className="cx-special-caution"><b>2개월 약정 예제</b><span>{guide.example}</span></aside></>
...
function FifthSixGuide({ guide }) {
  ...
  if (guide.kind === 'mortgage-scope') {
    ...
    body = <><div className="cx-form-legend">{guide.forms.map(x=><span key={x.code}><b>{x.code}</b>{x.name}</span>)}</div><div className="cx-form-matrix" style={{ '--cx-form-cols': guide.forms.length }}><div className="cx-form-matrix__head"><b>확인 항목</b>{guide.forms.map(x=><b key={x.code}>{x.code}</b>)}</div>{guide.items.map(x=><div className="cx-form-matrix__row" key={x.item}><strong>{x.item}</strong>{x.present.map((v,i)=><span className={v?'is-yes':'is-no'} key={guide.forms[i].code}><i>{guide.forms[i].code}</i><em aria-hidden>{v?'●':'—'}</em></span>)}</div>)}</div><aside className="cx-info-note">{guide.boundary}</aside></>
...
function NextSixGuide({ guide }) {
  ...
  if (guide.kind === 'agency-scope') {
    ...
    label = '투자지표 3개 계기판'
    body = <><div className="cx-metric-cards">{guide.metrics.map(x=><section className={`cx-metric--${x.color}`} key={x.name}><small>{x.unit}</small><strong>{x.name}</strong><span>{x.formula}</span><b>채택 {x.accept}</b></section>)}</div><div className="cx-equivalence">{guide.equivalence.map((x,i)=><span key={x}>{x}{i<guide.equivalence.length-1?<i>⇔</i>:null}</span>)}</div><ConceptStudySheet guide={guide} showIntro={false} /><div className="cx-reinvest"><span>{guide.reinvestment.npv}</span><span>{guide.reinvestment.irr}</span></div><aside className="cx-metric-sample"><b>예제</b><span>초기투자 {guide.sample.outlay} · 유입현가 {guide.sample.inflowPv} · NPV {guide.sample.npv} · PI {guide.sample.pi}</span><strong>{guide.sample.decision}</strong></aside></>
  ...
  return (
    <article className="cx-card cx-visual-card liquid-glass liquid-glass--tint cx-report-guide">
      <SectionBlock label="VISUAL PLUS" index={4}>
        ...
        <ConceptStudySheet guide={guide} showIntro={false} />
