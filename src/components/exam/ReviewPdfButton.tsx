...
export function ReviewPdfButton({
  ...
  if (!unlocked) {
    return (
      <button
        ...
        className="inline-flex items-center justify-center gap-2 rounded-[var(--radius-buttons)] border-[1.5px] border-mist bg-paper px-5 py-2 font-display text-body-sm font-medium text-fog"
      >
        📒 복습 PDF (프리미엄)
      </button>
    );
  ...
  if (total === 0) {
    return (
      <span className="inline-flex items-center justify-center gap-2 rounded-[var(--radius-buttons)] border-[1.5px] border-mist bg-paper px-5 py-2 font-display text-body-sm text-fog">
        📒 복습 PDF (북마크·메모 없음)
      </span>
    );
  }
