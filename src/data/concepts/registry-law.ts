...
const concepts: Concept[] = [
  // ───────── 공간정보관리법 ─────────
  {
    ...
    definition: "지목은 토지의 주된 용도에 따라 학교용지·수도용지·도로·유지·잡종지 등 법정된 종류 중 하나로 정해지며, 얼핏 비슷해 보이는 시설이라도 법령이 정한 기준에 따라 서로 다른 지목으로 갈린다.",
    intuition: "지목은 '땅의 용도를 한 단어로 요약한 이름'이라고 생각하면, 실제 시설과 지목 명칭을 짝짓는 문제를 직관적으로 풀 수 있다.",
    keyPoints: [
    ...
    pitfalls: "여객자동차터미널·운전학원·폐차장처럼 자동차 관련 독립시설은 잡종지로 등록되지만, 변전소·공항항만시설·도축장처럼 언뜻 유사해 보이는 시설들은 잡종지가 아니라는 점에서 혼동하기 쉽다.",
    example: "궤도를 갖춘 교통용 부지는 '도로'가 아니라 '철도용지'로 등록되며, 정수·배수 시설이 있는 부지는 '수도용지'로 등록된다.",
    questionRefs: [
  ...
  },
  {
    ...
    definition: "지번은 지적소관청이 지번부여지역별로 아라비아숫자로 순차 부여하며(임야대장·임야도는 숫자 앞에 '산'자 부기), 지적공부에 등록된 토지가 지형변화로 바다가 되어 원상회복이 불가능하면 지적소관청은 소유자에게 등록말소 신청을 통지하고 불응 시 직권으로 말소한다.",
    intuition: "지번부여, 지적도·임야도의 축척, 바다로 된 토지의 말소·회복은 모두 '지적소관청이 토지의 존재·범위를 어떻게 표시·유지·소멸시키는가'라는 같은 문제의 다른 국면이다.",
    keyPoints: [
    ...
    pitfalls: "임야도의 축척을 지적도와 같은 여러 종류로 착각하기 쉽지만, 임야도는 1/3000과 1/6000 두 가지 축척만 사용한다.",
    example: "지적공부에 등록된 토지가 지형변화로 바다가 되어 원상회복이 불가능한 경우, 소유자가 통지받고도 60일 내 등록말소를 신청하지 않으면 지적소관청이 직권으로 말소한다.",
    questionRefs: [
  ...
  },
  {
    ...
    definition: "지상경계는 둑·담장 등 구조물이나 경계점표지로 구분하며, 연접 토지 간 높낮이 차이가 있으면 그 구조물의 하단부, 공유수면매립지의 제방 등을 편입하는 경우에는 바깥쪽 어깨부분, 토지가 해면·수면에 접하는 경우에는 최대만조위 또는 최대만수위가 되는 선을 결정기준으로 한다.",
    intuition: "지상경계 결정기준은 눈에 보이는 구조물의 어느 지점을 경계로 볼 것인지를 유형별로 못박아 둔 규정이므로, 높낮이 차이·매립지·해안선처럼 상황별로 다른 기준점(상단/하단/어깨/만조위)을 짝지어 암기해야 한다.",
    keyPoints: [
    ...
    pitfalls: "해면·수면에 접하는 토지의 경계기준을 '평균해수면'으로 착각하기 쉽지만, 정확한 기준은 최대만조위 또는 최대만수위가 되는 선이다.",
    example: "공유수면매립지에서 제방을 토지에 편입해 등록하는 경우, 그 경사면이 아니라 바깥쪽 어깨부분이 지상경계의 결정기준이 된다.",
    questionRefs: [
  ...
  },
  {
    ...
    definition: "토지대장에 등록하는 면적은 원칙적으로 1제곱미터 단위로 정하되, 지적도 축척이 600분의 1인 지역과 경계점좌표등록부를 갖춘 지역은 0.1제곱미터 단위로 정하며, 끝수가 0.05㎡ 미만이면 버리고 초과하면 올린다.",
    intuition: "면적은 측량한 실제 수치를 그대로 등록하는 것이 아니라, 지역별 정밀도 기준(1㎡ 단위 vs 0.1㎡ 단위)에 맞춰 끝수 처리 규칙으로 다듬어 등록한다는 점이 핵심이다.",
    keyPoints: [
    ...
    pitfalls: "0.050㎡처럼 정확히 5로 끝나는 값을 무조건 올림 처리한다고 착각하기 쉽지만, 실제로는 앞자리 숫자의 홀짝에 따라 처리 방향이 갈리고 그 결과가 0.0㎡이 되면 최소단위로 올린다.",
    example: "600분의 1 축척 지역에서 신규등록 면적이 145.450㎡로 측정되면, 앞자리 4가 짝수이므로 버림 처리되어 토지대장에는 145.4㎡로 등록된다.",
    questionRefs: [
  ...
  },
  {
    ...
    definition: "지목을 지적도 및 임야도에 등록할 때는 부호로 표기해야 하며, 지목의 종류는 법령에 열거된 것에 한정되어 '선로용지'처럼 법정되지 않은 명칭은 사용할 수 없다.",
    intuition: "지목은 땅의 용도를 정형화한 28가지(예: 전·답·대·학교용지·철도용지 등) 카테고리로만 분류하도록 법에 못 박아 두었으므로, 목록에 없는 이름을 지어내는 지문은 항상 오답이라고 이해하면 된다.",
    keyPoints: [
    ...
    pitfalls: "실생활 용어(선로용지 등)를 법정 지목으로 착각하기 쉽다 — 반드시 법에 열거된 지목 명칭인지 확인해야 한다.",
    example: "철도 선로가 지나는 부지는 '선로용지'가 아니라 법정 지목인 '철도용지'로 등록된다.",
  },
  {
    ...
    definition: "토지대장·임야대장에는 지번·지목·면적 등이, 경계점좌표등록부에는 좌표가 등록되며, 각 지적공부마다 등록하는 항목이 다르다.",
    intuition: "지적공부가 여러 종류로 나뉘는 이유는 '표시할 정보의 성격'이 다르기 때문이므로, 어떤 정보가 어느 장부의 몫인지 짝짓는 것이 이 단원의 핵심이다.",
    keyPoints: [
    ...
    pitfalls: "'제명 앞에 (수치)라고 표시한다'는 서술은 틀렸다 — 제명 '끝'에 '(좌표)'라고 표시하는 것이 맞다.",
    example: "경계점좌표등록부가 있는 지역의 지적도라면, 도면 제명이 '○○동 지적도(좌표)'와 같은 형태로 표시된다.",
    questionRefs: [
  ...
  },
  {
    ...
    definition: "부동산종합공부는 토지의 표시·소유자, 건축물의 표시·소유자, 토지의 이용 및 규제, 부동산의 가격에 관한 사항을 지적소관청이 통합하여 관리하는 공부로, 등록사항에 잘못이 있으면 관리기관에 정정을 신청할 수 있다.",
    intuition: "부동산종합공부는 여러 행정기관이 따로 관리하던 토지·건물·가격·이용계획 정보를 한 장으로 모아 보여주는 통합 공부이므로, 원본 정보는 여전히 각 소관 기관이 관리하고 지적소관청은 이를 연계·관리할 뿐이라는 점이 핵심이다.",
    keyPoints: [
    ...
    pitfalls: "부동산종합공부가 원본 정보까지 새로 만든다고 착각하기 쉽지만, 실제로는 각 기관이 관리하는 원본 정보를 연계하여 통합 제공하는 공부일 뿐이다.",
    example: "토지의 가격 정보가 잘못 등록된 것을 발견한 소유자는 지적소관청에 부동산종합공부의 등록사항 정정을 신청할 수 있다.",
    questionRefs: [
  ...
  },
  {
    ...
    definition: "지적소관청은 토지의 이동에 따라 지상경계를 새로 정한 경우 경계점표지의 종류, 경계점 위치 설명도 등을 지상경계점등록부에 등록·관리해야 한다.",
    intuition: "지상경계는 현장에 실제로 표시된 경계점(말뚝, 표지 등)을 기준으로 하므로, 그 표지의 종류와 위치를 별도 장부로 남겨 두어야 나중에 분쟁이 생겨도 원래 경계를 확인할 수 있다.",
    keyPoints: [
    ...
    pitfalls: "지상경계점등록부의 등록사항을 토지대장 등 일반 지적공부의 등록사항과 혼동하기 쉽지만, 경계점 표지·사진 등 현장 표시 정보가 핵심이다.",
    example: "새로 지상경계를 정한 토지에 콘크리트 말뚝으로 경계점표지를 설치했다면, 그 표지의 종류가 지상경계점등록부에 등록된다.",
    questionRefs: [
  ...
  },
  {
    ...
    definition: "지적소관청은 지적공부가 멸실·훼손된 경우 지적공부의 등본, 측량 결과도, 부동산등기부 등본 등 관계 자료에 의해 지적공부를 복구해야 한다.",
    intuition: "복구는 없어진 장부를 다시 만드는 것이므로, 신뢰할 수 있는 다른 자료(등기부, 측량 결과, 토지이동정리 결의서 등)로 원래 내용을 증명할 수 있어야 복구 자료로 인정된다.",
    keyPoints: [
    ...
    pitfalls: "토지이용계획확인서처럼 관련 있어 보이는 서류를 복구 자료로 착각하기 쉽지만, 실제 복구자료 목록에는 열거되지 않은 자료다.",
    example: "화재로 지적공부가 소실된 경우, 지적소관청은 부동산등기부 등본과 측량 결과도 등을 근거로 지적공부를 복구한다.",
    questionRefs: [
  ...
  },
  {
    ...
    definition: "지적공부는 지적소관청 청사 내 지적서고에 보존해야 하며, 지목변경·지번변경·축척변경·합병 등으로 등기부와 지적공부의 내용이 달라지면 지적소관청이 관할 등기관서에 등기를 촉탁해야 하지만, 신규등록은 등기부 자체가 없어 촉탁 대상이 아니다.",
    intuition: "지적공부는 국가의 공적 장부이므로 어디에, 어떻게 보관하는지 자체가 법으로 정해져 있고, 등기부와 짝을 맞춰야 하는 변경사항(지목·지번·축척·합병)과 애초에 등기부가 없는 신규등록은 처리방식이 다르다는 점이 핵심이다.",
    keyPoints: [
    ...
    pitfalls: "신규등록도 등기촉탁 대상이라고 착각하기 쉽지만, 신규등록은 애초에 등기부 자체가 없으므로 촉탁 대상에서 제외된다.",
    example: "합병으로 지적공부의 표시가 바뀐 경우, 지적소관청은 지체 없이 관할 등기관서에 등기를 촉탁해야 한다.",
    questionRefs: [
  ...
  },
  {
    ...
    definition: "지적소관청은 도시개발사업 등이 준공되기 전에 사업시행자의 신청이 있으면 지적확정측량을 실시한 지역의 지번부여 방법을 준용해 지번을 부여할 수 있고, 합병하려는 토지는 지목·지번부여지역·소유자가 같아야 하며, 토지의 표시에 오류가 있으면 지적소관청이 직권으로 조사·측량하여 정정한다.",
    intuition: "지번·지목·경계 등 토지의 기본 표시사항을 새로 만들거나(등록전환) 합치거나(합병) 고칠 때(직권정정), 신청인의 뜻과 무관하게 지적소관청이 실체적 사실관계를 직접 확인해 등록을 좌우한다는 공통점이 있다.",
    keyPoints: [
    ...
    pitfalls: "합병은 소유자와 지목만 같다고 되는 것이 아니다. 서로 연접해야 하고, 지적도·임야도의 축척이 서로 다른 경우도 법정 합병 제한사유다.",
    example: "등록전환 대상 토지의 임야대장 면적과 실제 측량 면적의 차이가 허용범위를 넘으면, 지적소관청은 임야대장의 면적을 직권으로 정정한다.",
    questionRefs: [
  ...
  },
  {
    ...
    definition: "축척변경은 지적소관청이 토지소유자 3분의 2 이상의 동의를 받아 축척변경위원회의 의결과 시·도지사(또는 대도시 시장)의 승인을 거쳐 시행하며, 시행에 따른 면적 증감은 청산금으로 정산한다.",
    intuition: "축척변경은 지적도의 축척을 바꾸는 것뿐 아니라 그 과정에서 생기는 면적 증감을 돈으로 정산하는 절차까지 포함하므로, 동의 → 위원회 의결 → 승인 → 시행공고 → 청산금 → 확정공고의 흐름으로 이해해야 한다.",
    keyPoints: [
    ...
    pitfalls: "축척변경 '승인'을 축척변경위원회의 심의·의결사항으로 착각하기 쉽지만, 승인 권한은 위원회가 아니라 시·도지사 등에게 있다.",
    example: "토지소유자들이 축척변경을 신청하려면 시행지역 토지소유자 3분의 2 이상의 동의를 받아 지적소관청에 신청해야 한다.",
    questionRefs: [
  ...
  },
  {
    ...
    definition: "도시개발법·농어촌정비법과 대통령령상 토지개발사업의 시행자는 사업의 착수·변경·완료 사실을 지적소관청에 신고하고, 필요한 토지이동을 신청한다. 공동주택 부지 등 법 제87조의 사유에서는 관리인·공유자 대표자 등 법정 대위신청자가 토지소유자의 신청을 대신할 수 있다.",
    intuition: "대규모 개발사업 지역은 토지의 형질이 통째로 바뀌므로, 개별 토지이동 신청 대신 사업 단계(착수·변경·완료)에 맞춰 일괄 신고하는 특례를 두고 있다.",
    keyPoints: [
    ...
    pitfalls: "농어촌정비사업의 토지이동 시기를 '공사 착수 시'로 착각하기 쉽지만, 기준 시점은 '공사가 준공된 때'이다.",
    example: "택지개발사업이 환지를 수반하는 경우, 사업완료 신고를 함으로써 별도의 토지이동 신청을 갈음할 수 있다.",
    questionRefs: [
  ...
  },
  {
    ...
    definition: "지적소관청은 토지의 표시에 관한 변경등기가 필요한 경우 지체 없이 관할 등기관서에 등기를 촉탁해야 하고, 직권 정정·행정구역 개편 등으로 토지의 표시가 달라진 경우에는 토지소유자에게 그 사실을 통지해야 하며, 신규등록의 경우에는 등기부가 없으므로 지적소관청이 직접 조사하여 소유자를 등록한다.",
    intuition: "지적공부와 등기부는 서로 다른 기관이 관리하는 별개의 장부이므로, 한쪽의 표시가 바뀌면 다른 쪽에도 반영되도록 '촉탁'과 '통지'라는 연결 장치를 두고 있다.",
    keyPoints: [
    ...
    pitfalls: "토지소유자 정리의 근거를 '축척변경'이나 '등록전환'으로 착각하기 쉬운데, 등기부가 없어 지적소관청이 직접 조사하는 경우는 신규등록이다.",
    example: "행정구역 개편으로 어떤 토지가 다른 지번부여지역에 속하게 되어 지적소관청이 새 지번을 부여한 경우, 지적소관청은 그 토지소유자에게 이 사실을 통지해야 한다.",
    questionRefs: [
  ...
  },
  {
    ...
    definition: "지적측량의 측량기간은 5일, 측량검사기간은 4일을 원칙으로 하며, 지적기준점을 설치하여 측량하는 경우에는 점의 개수에 따라 기간이 가산된다.",
    intuition: "기준점 설치처럼 작업량이 늘어나는 요소가 있으면 그만큼 기간도 비례해서 늘어난다고 보면, 가산 규정의 취지가 자연스럽게 이해된다.",
    keyPoints: [
    ...
    pitfalls: "기간·점수 기준의 숫자를 바꿔서 내는 지문이 많으므로, 5일/4일/15점/4점 같은 구체적 수치를 정확히 암기해야 한다.",
    example: "지적기준점 19점을 설치하면 15점 이하 기본 가산 4일에 초과분 4점의 1일을 더하므로 측량 또는 검사기간에 총 5일을 가산한다.",
    questionRefs: [
  ...
  },
  {
    ...
    definition: "신규등록·등록전환·지적재조사사업에 따른 토지이동, 바다로 된 토지의 등록말소, 지적공부 복구·정정 등 토지의 표시가 달라지거나 확인이 필요한 경우에는 지적측량을 실시해야 하며, 지상건축물의 현황을 지적도·임야도상 경계와 대비해 표시하는 측량은 '지적현황측량'이라 한다.",
    intuition: "지적측량은 '토지의 표시를 새로 만들거나(신규등록·등록전환) 바꾸거나(정정·말소) 확인할(현황측량) 필요가 있을 때' 실시하는 것이므로, 단순히 지적공부를 정리만 하는 경우(예: 합병)에는 별도 측량이 필요 없다는 점과 대비해서 이해하면 된다.",
    keyPoints: [
    ...
    pitfalls: "토지의 합병도 지적공부가 바뀌니 측량이 필요하다고 착각하기 쉽지만, 합병은 새로 측량할 경계·좌표·면적이 없으므로 지적측량 실시 사유가 아니다.",
    example: "지상건축물이 지적도상 경계를 침범했는지 확인하려는 경우, 지적현황측량을 실시해 실제 현황과 등록된 경계를 대비한다.",
    questionRefs: [
  ...
  },
  {
    ...
    definition: "토지소유자 등 이해관계인은 지적측량수행자에게 지적측량을 의뢰할 수 있으나 지적측량성과 검사, 지적재조사사업에 따른 측량처럼 이해관계인의 의뢰 대상이 아닌 경우도 있으며, 지적기준점성과의 열람·등본 발급은 그 종류(지적삼각점·지적삼각보조점·지적도근점)에 따라 신청기관이 다르다.",
    intuition: "지적측량은 '누가 신청하는가'(이해관계인의 의뢰 vs 지적소관청 직권/검사)와 '그 결과를 누구에게 확인받는가'(기준점 종류별 신청기관)라는 두 갈래로 나뉘어 출제된다.",
    keyPoints: [
    ...
    pitfalls: "지적삼각점성과와 지적삼각보조점성과의 신청기관을 같다고 착각하기 쉽지만, 지적삼각점성과는 시·도지사 또는 지적소관청에, 지적삼각보조점성과는 지적소관청에만 신청한다.",
    example: "지적재조사사업에 따라 토지의 이동이 있어 측량이 필요한 경우, 이는 이해관계인이 지적측량수행자에게 의뢰해야 하는 사유가 아니다.",
    questionRefs: [
  ...
  },
  {
    ...
    definition: "중앙지적위원회는 위원장 1명과 부위원장 1명을 포함해 5명 이상 10명 이하의 위원으로 구성하며, 위원장은 국토교통부 지적업무 담당 국장이, 부위원장은 담당 과장이 맡고, 지적측량 적부심사 재심사·지적기술자 양성 등 법정 사항을 심의·의결한다.",
    intuition: "중앙지적위원회는 '지적 분야 전반의 정책·인사·재심사'를 다루는 최상위 위원회이므로, 구성원 수·소집 절차 같은 형식적 요건과 심의·의결사항의 범위를 정확히 구분해서 암기해야 한다.",
    keyPoints: [
    ...
    pitfalls: "회의 소집 통지기한을 '7일 전'으로 암기하기 쉽지만, 실제로는 '5일 전'까지 서면으로 통지해야 한다.",
    example: "지적측량 적부심사 의결에 불복해 재심사를 청구하면, 그 재심사는 지방지적위원회가 아니라 중앙지적위원회가 심의·의결한다.",
    questionRefs: [
  ...
  },
  {
    ...
    definition: "지적측량성과에 다툼이 있는 토지소유자·이해관계인·지적측량수행자는 관할 시·도지사를 거쳐 지방지적위원회에 적부심사를 청구할 수 있고, 지방지적위원회는 회부받은 날부터 60일 이내에 심의·의결하며, 시·도지사는 의결서를 받은 날부터 7일 이내에 청구인 등에게 통지해야 한다.",
    intuition: "적부심사는 '지방(1심) → 중앙(재심사)'의 2단계 구조를 가지며, 각 단계마다 청구·회부·심의·통지·불복의 기한이 촘촘히 정해져 있으므로 그 순서와 숫자를 하나의 흐름으로 암기해야 한다.",
    keyPoints: [
    ...
    pitfalls: "재심사 청구를 '시·도지사를 거쳐'로 착각하기 쉽지만, 재심사는 국토교통부장관을 거쳐 중앙지적위원회에 청구한다(최초 청구만 시·도지사를 거친다).",
    example: "지방지적위원회의 의결에 불복하는 자는 의결서를 받은 날부터 90일 이내에 국토교통부장관을 거쳐 중앙지적위원회에 재심사를 청구할 수 있다.",
    questionRefs: [
  ...
  // ───────── 부동산등기법 ─────────
  {
    ...
    definition: "법인의 등록번호는 주된 사무소 소재지 관할 등기소의 등기관이 부여하며, 등기기록의 열람은 이해관계인 여부와 관계없이 누구든지 청구할 수 있다.",
    intuition: "등기부는 공시(公示)를 목적으로 하는 공적 장부이므로 열람의 문턱을 낮게 두는 반면, 등기관의 처분에 대한 불복(이의신청)은 거래 안정을 위해 집행정지 효력을 주지 않는다는 점이 대조된다.",
    keyPoints: [
    ...
    pitfalls: "등록번호 부여기관을 '시장·군수·구청장'으로 잘못 기억하는 경우가 많다 — 부여 주체는 등기관이다.",
    example: "새로 설립된 법인이 부동산 등기를 위해 등록번호가 필요하면, 주된 사무소 소재지를 관할하는 등기소에서 이를 부여받는다.",
    questionRefs: [
  ...
  },
  {
    ...
    definition: "등기신청은 원칙적으로 등기권리자와 등기의무자가 공동으로 하지만, 등기의 성질상 상대방이 없거나 등기의무자의 협력을 요하지 않는 경우에는 등기권리자(또는 등기명의인)가 단독으로 신청할 수 있다.",
    intuition: "공동신청주의는 등기의 진정성을 담보하기 위한 원칙인데, 판결·상속·보존등기처럼 진정성이 이미 다른 방법으로 담보되거나 애초에 상대방이 없는 경우에는 단독신청을 허용해 절차를 간소화한다.",
    keyPoints: [
    ...
    pitfalls: "가등기말소는 가등기명의인이 단독으로 할 수 있지만, 근저당권 변경등기(채권최고액 감액 등)는 여전히 공동신청 대상이라는 점을 혼동하기 쉽다.",
    example: "한국토지주택공사가 토지를 수용한 경우, 그 수용에 따른 소유권이전등기는 사업시행자인 한국토지주택공사가 단독으로 신청할 수 있다.",
    questionRefs: [
  ...
  },
  {
    ...
    definition: "등기관은 등기신청이 부동산등기법 제29조 각 호의 각하사유에 해당하면 그 신청을 각하해야 하며, 그중 제2호 '사건이 등기할 것이 아닌 경우'는 실체법상 허용되지 않는 등기를 신청한 경우를 말한다.",
    intuition: "각하사유는 절차적 흠(신청정보 불비 등)과 실체적 흠('사건이 등기할 것이 아닌 경우')으로 나뉘는데, 후자는 등기가 실행되더라도 무효가 되는 등기를 애초에 걸러내기 위한 장치다.",
    keyPoints: [
    ...
    pitfalls: "공동상속인 중 일부가 자기 상속지분만 먼저 등기할 수 있다고 착각하기 쉽지만, 이는 부동산등기규칙 제52조의 '사건이 등기할 것이 아닌 경우'에 해당한다.",
    example: "공동상속인 A·B·C 중 A가 자기 상속지분만 상속등기하거나 농지에 전세권설정등기를 신청하면 등기관은 신청을 각하한다.",
    questionRefs: [
  ...
  },
  {
    ...
    definition: "등기관의 처분(각하 등)에 불복하는 자는 관할 지방법원에 이의신청을 할 수 있으나, 이의신청은 등기소에 이의신청서를 제출하는 방법으로 하며, 각하결정에 대한 이의신청이 인용되어 기록명령이 내려지면 등기관은 그 명령에 따라 등기를 실행한다.",
    intuition: "이의신청은 등기관의 처분이 위법·부당함을 다투는 절차이지 새로운 사실을 주장하는 재심사가 아니므로, 처분 당시 제출되지 않은 새 사실은 이의사유가 될 수 없다.",
    keyPoints: [
    ...
    pitfalls: "이의신청을 관할 지방법원에 직접 제출하는 것으로 착각하기 쉽지만, 실제로는 등기소에 제출한다.",
    example: "등기신청이 각하되어 이의신청을 하는 사이 다른 사람이 같은 부동산에 대해 별도로 등기를 신청하면, 그 신청은 이의신청과 무관하게 정상적으로 수리된다.",
    questionRefs: [
  ...
  },
  {
    ...
    definition: "등기신청 시 등기소에 제공해야 하는 신청정보(부동산 표시, 등기원인, 신청인 등)와 첨부정보(등기원인증명, 등기필정보, 인감증명 등)는 부동산등기법·규칙에서 정한 사항이며, 매매로 인한 소유권이전등기의 경우 토지의 면적과 같은 부동산 표시사항이 신청정보에 포함되어야 한다.",
    intuition: "신청정보는 '무엇을 등기해달라는 것인지'를 특정하는 정보이고 첨부정보는 '그 신청이 진실하다는 것'을 뒷받침하는 자료로, 등기소는 등기부에 공시될 필요가 없는 사항(대리인의 주민등록번호 등)까지는 신청정보로 요구하지 않는다.",
    keyPoints: [
    ...
    pitfalls: "등기필정보는 등기'의무자'만 제공하면 되는데, 등기권리자의 등기필정보까지 필요하다고 착각하기 쉽다.",
    example: "甲이 乙에게 토지를 매도하고 소유권이전등기를 신청할 때, 신청정보에는 그 토지의 면적을 기재해야 하지만 대리인의 주민등록번호까지 기재할 필요는 없다.",
    questionRefs: [
  ...
  },
  {
    ...
    definition: "채권자대위에 의한 등기신청, 등기원인증서에 대한 검인, 지체 없이 해야 하는 등기, 말소등기 시 승낙서를 첨부해야 하는 이해관계인 등은 등기신청 절차에서 문제되는 대표적인 특수 사례들이다.",
    intuition: "이 사례들은 모두 '누가 진짜 신청인·이해관계인인가'를 가려내는 문제로 수렴한다 — 대위등기에서는 채무자가 여전히 등기신청인이고, 말소등기에서는 순위상 후순위 권리자만 승낙서가 필요한 이해관계인이 된다.",
    keyPoints: [
    ...
    pitfalls: "채권자대위등기에서 등기신청인을 대위채권자로 착각하기 쉽지만, 신청인은 어디까지나 채무자이고 대위채권자는 그를 대신해 신청할 뿐이다.",
    example: "甲이 乙에 대한 채권을 보전하기 위해 乙을 대위하여 등기를 신청하는 경우, 등기완료통지는 실제 등기명의인이 되는 乙에게 이루어진다.",
    questionRefs: [
  ...
  },
  {
    ...
    definition: "건물의 표시등기는 소재·지번·종류·구조·면적 등 물리적 현황을 표제부에 반영한다. 건축물대장에 합병등록이 되면 소유권 등기명의인은 1개월 이내 합병등기를 신청해야 하고, 기존 권리의 동일성과 순위를 잇는 저당권 이전등기는 부기등기로 한다.",
    intuition: "'표시에 관한 등기'는 부동산 자체의 물리적 현황(면적·구조·합병 등)을 반영하는 등기이고, 부기등기는 기존 순위를 그대로 유지하면서 덧붙이는 등기라는 두 축을 구분해서 이해하면 된다.",
    keyPoints: [
    ...
    pitfalls: "건물합병은 표시변경이고 저당권 이전은 권리변동이다. 둘을 같은 등기 유형으로 묶지 말고, 후자는 기존 저당권의 순위를 잇기 때문에 부기등기로 한다.",
    example: "인접한 두 건물이 건축물대장상 하나로 합병 등록되었다면, 소유권 등기명의인은 합병등록일로부터 1개월 이내에 건물합병등기를 신청해야 한다.",
  },
  {
    ...
    definition: "미등기건물을 매수한 자는 매수인 명의로 바로 보존등기를 할 수 없고 원소유자(신축자) 명의로 보존등기 후 이전등기를 해야 하며, 등기의무자가 사망하면 그 단독상속인이 자신을 등기의무자로 하여 피상속인에서 direct 등기권리자로의 이전등기를 신청할 수 있다.",
    intuition: "등기제도는 '실제 권리변동의 연속된 과정'을 그대로 기록하는 것을 원칙으로 하므로, 원시취득자를 건너뛰고 바로 최종 매수인 명의로 등기하는 것은 원칙적으로 허용되지 않지만, 상속처럼 법률로 당연히 승계되는 경우에는 중간 단계(상속등기)를 생략하고 직접 등기할 수 있는 예외가 인정된다.",
    keyPoints: [
    ...
    pitfalls: "등기의무자가 사망하면 반드시 상속등기를 먼저 거쳐야 한다고 착각하기 쉽다 — 상속인은 자신을 등기의무자로 하여 직접 이전등기를 신청할 수 있다.",
    example: "甲이 신축한 미등기건물을 매수한 乙은, 甲 명의로 소유권보존등기를 마친 뒤 乙 앞으로 소유권이전등기를 해야 한다.",
    questionRefs: [
  ...
  },
  {
    ...
    definition: "일정한 금액을 목적으로 하지 않는 채권을 담보하는 저당권설정등기를 신청할 때는 그 채권의 평가액을 신청정보로 제공해야 하며, 3개의 부동산이 공동담보로 제공되는 경우가 아니라 5개 이상인 경우에만 등기관이 공동담보목록을 작성한다.",
    intuition: "저당권 등기사항은 '그 채권이 정확히 얼마인지, 누구의 채무를 담보하는지'를 등기부에 명확히 남기려는 취지이므로, 금액이 불확정적인 채권이나 여러 부동산에 걸친 공동담보처럼 특수한 경우마다 별도의 기재 규칙을 두고 있다고 이해하면 된다.",
    keyPoints: [
    ...
    pitfalls: "공동담보목록 작성 기준을 '부동산 3개 이상'으로 착각하기 쉽다 — 정확히는 '5개 이상'인 경우에만 작성한다.",
    example: "채권최고액을 확정할 수 없는 특수한 채권을 담보로 저당권을 설정할 때는, 그 채권의 평가액을 신청정보로 등기소에 제공해야 한다.",
    questionRefs: [
  ...
  },
  {
    ...
    definition: "지상권·지역권설정등기에는 그 설정의 목적을 기록해야 하며, 지상권설정등기의 범위가 토지 일부인 경우에는 그 부분을 표시한 지적도(토지대장이 아님)를 첨부정보로 제공해야 하고, 임차권설정등기에는 차임 및 임차보증금(있는 경우)을 등기사항으로 기록한다.",
    intuition: "용익권 등기사항은 대체로 '그 권리가 무엇을 목적으로, 어느 범위에 미치는지'를 등기부에 남기는 데 초점이 있으므로, 각 권리별로 요구되는 특유의 기재사항(지상권의 목적, 지역권의 목적, 임차권의 차임 등)을 서로 구분해서 암기해야 한다.",
    keyPoints: [
    ...
    pitfalls: "지상권 범위가 토지 일부인 경우의 첨부정보를 '토지대장'으로 착각하기 쉽다 — 정확히는 '지적도'다.",
    example: "한 필지 토지의 절반에만 지상권을 설정하는 경우, 그 부분을 표시한 지적도를 첨부정보로 등기소에 제공해야 한다.",
    questionRefs: [
  ...
  },
  {
    ...
    definition: "수익자는 수탁자를 대위하여 신탁등기를 신청할 수 있고, 신탁등기의 말소등기는 수탁자가 단독으로 신청할 수 있으며, 하나의 부동산에 수탁자가 여러 명이면 등기관은 그 신탁부동산이 합유임을 기록해야 한다.",
    intuition: "신탁등기 절차는 '수탁자가 주된 신청인이되, 수익자에게도 보완적 신청권을 준다'는 구조와 '신탁재산이 수탁자 개인 재산과 섞이지 않도록 합유로 공시한다'는 원칙으로 이해하면 대부분의 지문을 판단할 수 있다.",
    keyPoints: [
    ...
    pitfalls: "신탁재산에서 벗어난 권리에 대한 신탁등기 말소를 '별도로' 신청해야 한다고 착각하기 쉽다 — 권리이전등기 신청과 동시에 해야 한다.",
    example: "부동산이 신탁재산에서 벗어나 제3자에게 이전되는 경우, 그 이전등기를 신청할 때 신탁등기 말소신청도 함께 해야 한다.",
    questionRefs: [
  ...
  },
  {
    ...
    definition: "합유등기에는 합유지분을 표시하지 않으며, 미등기 부동산의 공유자 중 1인은 자기 지분만이 아니라 부동산 전체에 대해서만 보존등기를 신청할 수 있다. 법인 아닌 사단 명의 부동산을 매매로 이전등기할 때는 특별한 사정이 없는 한 사원총회 결의를 증명하는 정보를 제출해야 한다.",
    intuition: "합유는 '지분이 있으나 자유롭게 처분할 수 없는' 공동소유 형태이므로 등기부에 지분을 아예 표시하지 않고, 미등기 공유부동산의 보존등기는 '전체를 하나로' 등기해야 한다는 원칙으로 이해하면 된다.",
    keyPoints: [
    ...
    pitfalls: "미등기 공유부동산의 공유자 1인이 자기 지분만 보존등기할 수 있다고 착각하기 쉽다 — 반드시 부동산 전체에 대해서만 신청할 수 있다.",
    example: "미등기 토지를 3인이 공유하는 경우, 그중 1인이 자신의 지분만 따로 떼어 보존등기를 신청할 수는 없고 토지 전체에 대해 보존등기를 신청해야 한다.",
    questionRefs: [
  ...
  },
  {
    ...
    definition: "환매특약등기, 지상권의 이전등기, 등기명의인표시의 변경등기, 지상권 위에 설정한 저당권의 이전등기는 이해관계인의 승낙 여부와 무관하게 부기등기로 하지만, 근저당권의 채권최고액을 증액하는 변경등기는 후순위 권리자 등 이해관계 있는 제3자의 승낙이 없으면 부기등기로 할 수 없다.",
    intuition: "부기등기는 기존 등기의 순위를 그대로 유지하면서 덧붙이는 등기이므로, 그 변경이 다른 이해관계인에게 불리한 영향을 주지 않는 경우(단순 표시변경, 이전등기 등)에는 승낙 없이도 부기등기로 처리되지만, 채권최고액 증액처럼 후순위자에게 불리해질 수 있는 변경은 그 승낙을 받아야만 순위를 그대로 유지하는 부기등기로 할 수 있다.",
    keyPoints: [
    ...
    pitfalls: "근저당권 채권최고액 증액 변경등기도 승낙 없이 부기등기로 할 수 있다고 착각하기 쉽다 — 이해관계 있는 제3자의 승낙이 필요하다.",
    example: "근저당권의 채권최고액을 증액하려는 경우, 후순위 저당권자 등 이해관계 있는 제3자의 승낙을 받아야만 그 변경등기를 부기등기로 할 수 있다.",
    questionRefs: [
  ...
  },
  {
    ...
    definition: "환매특약등기에는 매수인이 지급한 대금과 매매비용을 기록하고, 등기원인에 환매기간 약정이 있으면 그 기간도 기록한다. 채권최고액이나 이자지급시기는 등기사항이 아니다.",
    intuition: "환매특약등기는 '나중에 되사올 때 정산할 기준 금액(매매대금·매매비용)'을 공시하는 것이 목적이므로, 근저당권의 채권최고액이나 이자 조건처럼 신용거래에 특유한 사항은 애초에 등기사항이 아니라고 구분하면 된다.",
    keyPoints: [
    ...
    pitfalls: "대금·매매비용만 외우고 약정된 환매기간을 빠뜨리기 쉽다. 기간은 언제나가 아니라 등기원인에 약정이 있을 때 기록한다.",
    example: "매수인이 3억원을 지급하고 매매비용 500만원, 환매기간 3년을 약정했다면 세 항목을 기록한다. 채권최고액과 이자지급시기는 기록하지 않는다.",
    questionRefs: [
  ...
  },
  {
    ...
    definition: "등기원인에 그러한 약정이 있더라도 등기기록에 기록할 수 없는 사항이 법령상 정해져 있으며, 수용으로 인한 등기·말소등기 등 각종 등기 유형마다 별도의 요건과 효력이 규정되어 있다.",
    intuition: "이 카드는 위의 개념 카드들(소유권·저당권·용익권·신탁·공유·부기등기·환매특약)로 깔끔히 분류되지 않는 등기법 총론 성격의 개별 지문들을 모아둔 것이므로, 하나의 원리로 묶이지는 않지만 실전에서 반복 출제되는 개별 규정들로 알아두어야 한다.",
    keyPoints: [
    ...
    pitfalls: "당사자 간 약정이 있으면 어떤 사항이든 등기기록에 기록할 수 있다고 착각하기 쉽다 — 법령상 기록할 수 없는 사항으로 정해진 것은 약정으로도 등기할 수 없다.",
    example: "당사자가 특약으로 등기원인에 특정 조건을 붙였더라도, 그 조건이 법령상 등기기록에 기록할 수 없는 사항이라면 등기관은 이를 등기부에 기록하지 않는다.",
    questionRefs: [
  ...
  },
  {
    ...
    definition: "가등기는 채권적 청구권(정지조건부 청구권 포함)을 보전하기 위해 신청할 수 있지만 해제조건부 청구권이나 물권적 청구권, 소유권보존등기를 위한 가등기는 허용되지 않으며, 가등기권리자와 의무자가 공동으로 신청하거나 의무자의 승낙·가처분명령이 있으면 권리자가 단독으로 신청할 수 있고, 본등기의 물권변동 효력은 가등기 시로 소급하지 않는다.",
    intuition: "가등기는 장래에 본등기를 할 청구권을 미리 순위 보전해주는 제도이므로, 이미 확정적으로 존재하는 물권적 청구권이나 애초에 순위 다툼이 없는 보존등기에는 필요하지도, 허용되지도 않는다는 원칙에서 세부 규정들이 파생된다.",
    keyPoints: [
    ...
    pitfalls: "본등기의 물권변동 효력이 가등기 시로 소급한다고 착각하기 쉽지만, 실제로는 순위만 가등기 시로 소급할 뿐 물권변동의 효력 자체는 본등기 시에 발생한다.",
    example: "정지조건부로 소유권이전등기청구권을 취득하기로 한 경우 그 청구권을 보전하기 위한 가등기를 할 수 있지만, 해제조건부인 경우에는 가등기를 할 수 없다.",
    questionRefs: [
  ...
  },
  {
    ...
    definition: "소유권이전청구권보전 가등기에 의해 본등기가 이루어지면 그 가등기 후에 마쳐진 소유권이전등기·제한물권(지상권·지역권·저당권·임차권 등) 설정등기는 직권말소되지만 가등기상 권리를 목적으로 한 가압류·가처분등기는 말소되지 않으며, 용익물권(지상권 등) 설정청구권보전 가등기에 의해 본등기가 이루어지면 그 본등기와 양립할 수 없는 이후의 용익권 설정등기만 말소되고 저당권·압류·가압류 등은 말소되지 않는다.",
    intuition: "직권말소 범위는 결국 본등기로 실현되는 권리와 양립할 수 없는 등기인가로 판단하면 된다 — 소유권이전 본등기는 거의 모든 후행 등기와 양립 불가능하지만, 지상권 같은 용익권 본등기는 저당권·압류처럼 소유권을 전제로 한 등기와는 양립할 수 있다.",
    keyPoints: [
    ...
    pitfalls: "지상권설정 가등기에 의한 본등기 시에도 저당권이나 압류등기가 말소된다고 착각하기 쉽지만, 용익권 가등기의 본등기는 그와 양립 불가능한 용익권 설정등기만 말소시키고 저당권·압류는 말소시키지 않는다.",
    example: "소유권이전청구권보전 가등기에 의해 본등기가 되면 그 사이 마쳐진 저당권설정등기는 직권말소되지만, 그 가등기상 권리를 목적으로 한 가압류등기는 말소되지 않는다.",
    questionRefs: [
